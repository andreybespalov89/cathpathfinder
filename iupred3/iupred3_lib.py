...
def _require_savgol_filter():
    try:
        from scipy.signal import savgol_filter
    except Exception as exc:
        raise ImportError(
            "SciPy (scipy.signal.savgol_filter) is required for medium smoothing. "
            "Run with the Python interpreter where SciPy is installed, e.g. "
            f"'{sys.executable} iupred3.py ...', or install SciPy for this interpreter."
        ) from exc
